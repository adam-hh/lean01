...
static uint64_t SHA512224_H0[ ] = {
    0x8C3D37C819544DA2LL, 0x73E1996689DCD4D6LL, 0x1DFAB7AE32FF9C82LL,
    0x679DD514582F9FCFLL, 0x0F6D2B697BD44DA8LL, 0x77E36F7304C48942LL,
    0x3F9D85A86A1D36C8LL, 0x1112E6AD91D692A1LL
};
static uint64_t SHA512256_H0[ ] = {
    0x22312194FC2BF72CLL, 0x9F555FA3C84C64C2LL, 0x2393B86B6F53B151,
    0x963877195940EABDLL, 0x96283EE2A88EFFE3LL, 0xBE5E1E2553863992,
...
int SHA512_224Input(SHA512Context *context,
    const uint8_t *message_array, unsigned int length)
{
  return SHA512Input(context, message_array, length);
}
int SHA512_256Input(SHA512Context *context,
    const uint8_t *message_array, unsigned int length)
{
  return SHA512Input(context, message_array, length);
}
...
int SHA512_224FinalBits(SHA512Context *context,
                    uint8_t message_bits, unsigned int length)
{
  return SHA512FinalBits(context, message_bits, length);
}
int SHA512_256FinalBits(SHA512Context *context,
                    uint8_t message_bits, unsigned int length)
{
  return SHA512FinalBits(context, message_bits, length);
}
...
{
  return SHA384_512Reset(context, SHA512224_H0);
}
int SHA512_256Reset(SHA512Context *context)
{
  return SHA384_512Reset(context, SHA512256_H0);
}
        ...
        unsigned int length)
{
  ...
  if ((length + curIdx >= SHA512_Message_Block_Size) &&
      (SHA384_512AddLength(context, (SHA512_Message_Block_Size - curIdx) << 3) == shaSuccess)) {
    memcpy(context->Message_Block + curIdx, message_array, SHA512_Message_Block_Size - curIdx);
    message_array += SHA512_Message_Block_Size - curIdx;
    SHA384_512ProcessMessageBlock(context);
    length -= (SHA512_Message_Block_Size - curIdx);
  } else if (SHA384_512AddLength(context, length << 3) == shaSuccess) {
  ...
  while (length) {
    if ((length >= SHA512_Message_Block_Size) &&
        (SHA384_512AddLength(context, SHA512_Message_Block_Size << 3) == shaSuccess)) {
      memcpy(context->Message_Block, message_array, SHA512_Message_Block_Size);
      SHA384_512ProcessMessageBlock(context);
      message_array += SHA512_Message_Block_Size;
      length -= SHA512_Message_Block_Size;
    } else if (SHA384_512AddLength(context, length << 3) == shaSuccess) {
...
int SHA512_224Result(SHA512Context *context,
    uint8_t Message_Digest[SHA512224HashSize])
{
  return SHA384_512ResultN(context, Message_Digest, SHA512224HashSize);
}
int SHA512_256Result(SHA512Context *context,
    uint8_t Message_Digest[SHA512256HashSize])
{
  return SHA384_512ResultN(context, Message_Digest, SHA512256HashSize);
}
...
#ifdef USE_32BIT_ONLY
static int SHA384_512Reset(SHA512Context *context,
                           uint32_t H0[SHA512HashSize >> 3])
#else /* !USE_32BIT_ONLY */
static int SHA384_512Reset(SHA512Context *context,
                           uint64_t H0[SHA512HashSize >> 3])
#endif /* USE_32BIT_ONLY */
{
  int i;
  if (!context) return shaNull;
  ...
  for (i = 0; i < SHA512HashSize >> 2; i++)
    context->Intermediate_Hash[i] = H0[i];
#else /* !USE_32BIT_ONLY */
  context->Length_High = context->Length_Low = 0;

  for (i = 0; i < (SHA512HashSize >> 3); i++)
    context->Intermediate_Hash[i] = H0[i];
#endif /* USE_32BIT_ONLY */
...
#else /* !USE_32BIT_ONLY */
  if (HashSize == SHA512224HashSize)
    HashSize = 32;
	lcnt = HashSize >> 3;
	for (i = 0; i < lcnt; i++) {
		Message_Digest[(i << 3) + 0] = (context->Intermediate_Hash[i] >> 56);
		Message_Digest[(i << 3) + 1] = (context->Intermediate_Hash[i] >> 48);
		Message_Digest[(i << 3) + 2] = (context->Intermediate_Hash[i] >> 40);
		Message_Digest[(i << 3) + 3] = (context->Intermediate_Hash[i] >> 32);
		Message_Digest[(i << 3) + 4] = (context->Intermediate_Hash[i] >> 24);
		Message_Digest[(i << 3) + 5] = (context->Intermediate_Hash[i] >> 16);
		Message_Digest[(i << 3) + 6] = (context->Intermediate_Hash[i] >> 8);
		Message_Digest[(i << 3) + 7] = (context->Intermediate_Hash[i]);
	}
	/*
