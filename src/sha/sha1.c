    ...
    uint8_t Message_Digest[SHA1HashSize])
{
  ...
  if (!context->Computed)
    SHA1Finalize(context, 0x80);    /* use default pad byte 0x80 */

  /* Machine byte order unrelated operation */
  for (i = 0; i < (SHA1HashSize >> 2); i++) {
    Message_Digest[(i << 2) + 0] = (context->Intermediate_Hash[i] >> 24);
    Message_Digest[(i << 2) + 1] = (context->Intermediate_Hash[i] >> 16);
    Message_Digest[(i << 2) + 2] = (context->Intermediate_Hash[i] >> 8);
    Message_Digest[(i << 2) + 3] = (context->Intermediate_Hash[i]);
  }
  /*
...
static void SHA1ProcessMessageBlock(SHA1Context *context)
{
  ...
  for (t = t4 = 0; t < 16; t++, t4 += 4) {
    ...
    W[t] |= ((uint32_t)context->Message_Block[t4 + 1]) << 16;
    W[t] |= ((uint32_t)context->Message_Block[t4 + 2]) << 8;
    W[t] |= ((uint32_t)context->Message_Block[t4 + 3]);
  }
...
static void ntohSHA1(SHA1Context *context)
{
  uint32_t *ptr, tmp;
  uint8_t *p;

  ptr = (uint32_t *)(context->Message_Block);
  for(int i = 0; i < (sizeof(context->Message_Block) >> 2); i++) {
    tmp = ptr[i];
    p = (uint8_t *)(&ptr[i]);
    p[0] = (uint8_t)(tmp >> 24);
    p[1] = (uint8_t)(tmp >> 16);
